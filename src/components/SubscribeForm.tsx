  ...
  if (status === 'success') {
    return (
      <div className={`flex items-center justify-center gap-3 ${className}`}>
        ...
        <span className="text-ivory/80 text-sm tracking-wide">
  ...
  return (
    ...
    >
      <input
        ...
        required
        className="
          flex-1 bg-white/[0.04] border border-white/10
          text-ivory placeholder-ivory/25
          px-5 py-3.5 text-sm font-sans outline-none
          focus:border-gold/40 transition-colors duration-200
        "
      />
      <button
        type="submit"
      ...
      {status === 'error' && (
        <p className="text-red-400/80 text-xs text-center sm:text-left col-span-full mt-1">
          {message}
        </p>
      )}
    </form>
